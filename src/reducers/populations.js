  ...
  switch (action.type) {
    case REQUEST_POPULATION_FULFILLED:
      ...
      if (! pops.has(action.payload[1].qualityReportId)) {
        qrProps[action.payload[1].population] = [{page: 1, patients: response.patients}];
      } else {
        qrProps = pops.get(action.payload[1].qualityReportId);
        let individualPop = qrProps[action.payload[1].population];
        const pageNumber = (response.populationQuery.offset / PATIENTS_PER_PAGE) + 1;
        if (individualPop) {
          qrProps[action.payload[1].population] = qrProps[action.payload[1].population].concat({page: pageNumber, patients: response.patients});
        } else {
